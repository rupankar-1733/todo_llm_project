...
export default function App() {
  ...
  // Check for saved token and dark mode on mount
  useEffect(() => {
  ...
  // Force reload alerts when tasks change
  useEffect(() => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const cleanError = String(e).replace('Error: ', '').replace(/[{}"]/g, '').replace('detail:', '').trim()
      setLoginError(cleanError)
  ...
  const loadDueSoon = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        console.log('Due soon data:', data) // Debug log
        setDueSoon(data)
      }
    } catch (e) {
      console.error('Due soon error:', e)
    }
  }
  ...
  const getCategoryEmoji = (category) => {
    const emojis = {
      ...
      education: '📚', finance: '💰', travel: '✈️', home: '🏠', other: '📋'
    }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-white text-opacity-90 hidden sm:block">AI-powered task management</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white bg-opacity-20 border border-white border-opacity-30 rounded-2xl p-6 backdrop-blur-lg animate-fadeIn">
              ...
              <textarea
                className="w-full bg-white bg-opacity-30 border border-white border-opacity-40 text-white placeholder-white placeholder-opacity-70 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-white focus:ring-opacity-50 focus:outline-none resize-none backdrop-blur-sm transition-all"
                rows={3}
                placeholder="Try: 'Create a high priority work task to finish report by tomorrow'"
                value={message}
                onChange={e => setMessage(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && e.ctrlKey && callChat()}
              />
              <div className="flex flex-col sm:flex-row gap-3 mt-4">
                <button
                  onClick={callChat}
                  disabled={loading}
                  className="flex-1 px-6 py-3 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 text-white font-medium hover:from-pink-600 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg transform hover:scale-105"
                >
                  {loading ? '⏳ Processing...' : '🚀 Send Message'}
                </button>
                <button
                  onClick={loadTasks}
                  className="sm:w-auto px-6 py-3 rounded-xl bg-white bg-opacity-30 border border-white border-opacity-40 text-white font-medium hover:bg-opacity-40 backdrop-blur-sm transition-all transform hover:scale-105"
                >
                  🔄 Refresh
                </button>
              </div>
            ...
            <div className="bg-white bg-opacity-20 border border-white border-opacity-30 rounded-2xl p-6 backdrop-blur-lg">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="none" className="text-gray-900">No sorting</option>
                    <option value="due_date" className="text-gray-900">Due Date</option>
                    <option value="priority" className="text-gray-900">Priority</option>
                    <option value="created" className="text-gray-900">Recently Created</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white bg-opacity-20 border border-white border-opacity-30 rounded-2xl p-6 backdrop-blur-lg">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                ...
                >
                  ...
                  <option value="health" className="text-gray-900">❤️ Health</option>
            ...
            <div className="bg-white bg-opacity-20 border border-white border-opacity-30 rounded-2xl p-6 backdrop-blur-lg">
              ...
              {displayTasks.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-white text-opacity-90 font-medium">No tasks found. Create one using chat!</p>
                </div>
              ) : (
                <div className="space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            ...
                            <span className={`text-xs px-3 py-1 rounded-full text-white font-medium ${getPriorityGradient(t.priority)}`}>
